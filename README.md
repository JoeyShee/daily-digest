# Daily Digest

辉哥的商业感知入口。首页不再承担“把所有新信息展示出来”，而是先用 Repricing 分析把散乱信号压缩成少量判断：**什么正在被重新定价，以及它对 Build / 10x 意味着什么。**

原始 RSS、社区、课程、案例和想法继续保留在后台与专项频道，作为发现和求证材料；它们不再默认要求用户逐条阅读。

默认信息层级：

1. 一个今日主判断；
2. 最多少量候选判断；
3. 按需展开的证据层；
4. 原始信号库与历史专项频道。

Repricing 正式报告路径：`~/.hermes/data/repricing/reports/YYYY-MM-DD.md`。

## 双创研究

“双创研究”是 10x 投机研究系统的公开阅读层。首页分为“当前行动 / 本周变化 / 公司库”，
普通研究在后台持续积累，只有改变行动的证据才即时提醒。每天的成熟判断仍保留为日期归档页，内容来自
`~/Documents/10x投机/deliveries/YYYY-MM-DD-*.md`，而不是把原始信号、研究台账或未验证猜测直接公开。

固定阅读结构是：当前状态、今天改变了什么、可能的错价、最强反证和当前行动。没有足够证据时，允许明确发布“今日无重要变化”，不为保持日更而制造机会。

自动发布入口：`./publish_dual_innovation.sh`。所有公开栏目统一通过
`./publish_to_personal_site.sh` 触发个人站部署；GitHub 仓库仅保留公开产物的版本记录。

🔗 [jsbuildslowly.com/digest](https://jsbuildslowly.com/digest/)

旧 GitHub Pages 地址只保留兼容跳转，不再作为正式发布出口。
